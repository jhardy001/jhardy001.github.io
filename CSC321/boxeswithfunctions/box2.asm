...
;; !!!!!!!!!!    DISCLAIMER: CODE NOT FINISHED YET       !!!!!!!!!!
...
section .data                                   ; Initialized data segment, mostly used for constants
 ...
 MessageVolumn        db "The volume is: "               ;    These have memory locations.
...
alignb 8
 ...
 Term3          resq 1                          ; Second term of addition
 Volume         resq 1                          ; sum of the two terms
 SurfaceArea    resq 1                          ; sum of the two terms
...
;;; Function calcVolume
;;; Takes the length (ECX), width (EDX), and height (R8D) of a box
;;; Returns the volume (in EAX)
;;; Handles negative inputs (not that they make sense, but...)
calcVolume: ;; Beginning of function is just a label
   ;; Entry code (preamble)
   ...
   mov [rsp+16], rdx ; Parameter 2 (width)
   mov [rsp+24], r8 ; Parameter 3 (height)
   ;; Save any other non-volatile register(s) used

   ;; Calculate the volume
   mov eax, ecx
   imul edx
   imul r8d
   ;; Product is in EAX (the top 32 bits are in EDX, but we'll ignore those)

   ;; Exit code (epilogue)
   ;; Restore saved, non-volatile register(s) (if any)
...
;;; Returns the volume (in EAX)
;;; Handles negative inputs (not that they make sense, but...)
calcSurfaceArea: ;; Beginning of function is just a label
   ;; Entry code (preamble)
   ;; Copy parameters into shadow space.  This is a little bit paranoid,
   ;;   but not very--if I ever call another procedure from within this
   ;;   one, I will need this.
   ...
   mov [rsp+16], rdx ; Parameter 2 (width)
   mov [rsp+24], r8 ; Parameter 3 (height)
   mov [rsp+32], r9 ; Parameter 3 (height)
   mov [rsp+40], r10 ; Parameter 3 (height)
   mov [rsp+48], r11
   ;; Save any other non-volatile register(s) used
...
;; Use a function to calculate the volume
;; eax := volume(length, width, height)
 sub   rsp, 32                                  ; Shadow space
 mov   ECX, dword [REL Term1]                   ; 1st parameter
 mov   EDX, dword [REL Term2]                   ; 2nd parameter
 mov   R8d, dword [REL Term3]                   ; 3rd parameter
 call  calcVolume
 mov   [REL Volume], eax                        ; Store the return value
 add   rsp, 32                                  ; Dump the shadow space

;; Print the label for the Volume
 sub   RSP, 32 + 8 + 8                          ; Shadow space + 5th parameter + align stack
                                                ; to a multiple of 16 bytes (MS x64 calling convention)
 mov   RCX, qword [REL StdOutHandle]            ; 1st parameter
...
;; Use a function to calculate the surface area
;; eax := surface area(length, width, height)
 sub   rsp, 32                                  ; Shadow space
 mov   ECX, dword [REL Term1]                   ; 1st parameter
 mov   EDX, dword [REL Term2]                   ; 2nd parameter
 mov   R8d, dword [REL Term3]                   ; 3rd parameter
 mov   R9D, 2                                   ; 4th parameter
 mov   R10D, ecx                                ; 5th parameter -- in place of eax for now
 mov   r11, 1
call calcSurfaceArea
mov   [REL SurfaceArea], eax                        ; Store the return value
 add   rsp, 32                                  ; Dump the shadow space

;; Print the label for the SA
 sub   RSP, 32 + 8 + 8                          ; Shadow space + 5th parameter + align stack
                                                ; to a multiple of 16 bytes (MS x64 calling convention)
 mov   RCX, qword [REL StdOutHandle]            ; 1st parameter
...
;; Convert the SA to a string
 sub  rsp, 32                                   ; Shadow space
 mov  ecx, [REL SurfaceArea]                          ; Parameter 1: number
 lea  rdx, [REL InputSpace]                     ; Parameter 2: address of string space
 call int2str
 mov  [REL BytesRead], eax                      ; Store the length of the string written
 add  rsp, 32                                   ; Dump the shadow space
